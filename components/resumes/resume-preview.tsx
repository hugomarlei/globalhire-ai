...
export function ResumePreview({ data, printable = false }: { data: ResumeData; printable?: boolean }) {
  ...
  return (
    <article
      className={cn(
        "resume-print min-h-[900px] bg-white text-slate-950 shadow-sm print:min-h-0 print:shadow-none",
        printable ? "w-full" : "mx-auto w-full max-w-[820px]",
        sidebar ? "grid grid-cols-[0.72fr_1.28fr]" : "p-7 sm:p-9",
        data.template === "professional" && "border-t-[10px]",
        compact && "font-serif"
      )}
      style={{ borderColor: data.template === "professional" ? color : undefined }}
    >
      <div className={cn(sidebar ? "space-y-4 p-6 text-white" : "contents")} style={sidebar ? { backgroundColor: color } : undefined}>
      ...
      <div className={cn("space-y-5", sidebar && "p-7")}>
        {data.summary ? (
