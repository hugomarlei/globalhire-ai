...
export default async function GeneratorPage({ searchParams }: { searchParams?: Promise<{ tipo?: string }> }) {
  ...
  return (
    <div className="grid gap-4">
      <div>
        <h1 className="text-3xl font-semibold text-ink dark:text-white">{context.title}</h1>
        <p className="mt-2 text-sm text-graphite/65 dark:text-white/60">{context.subtitle}</p>
      </div>
